...
class transformer_RL():
    ...
    def likelihood(self, target):
        ...
        con_token = target[:, 0:2]
        x = torch.cat((start_token, target[:, 2:-1]), 1)
        y_target = target[:, 2:].contiguous().view(-1)
        ...
        # ## 计算准确率, validate时添加
        # correct =0
        # max_indices = torch.argmax(logits.view(-1, self.voc.vocab_size), dim=1)
        # correct += (max_indices.view(-1,seq_length) == y_target.view(-1,seq_length)).sum()/max_indices.shape[0]
    ...
    def generate(self, batch_size, max_length=140, con_token_list= ['high_QED', 'good_SA']):
        """
               Sample a batch of sequences
