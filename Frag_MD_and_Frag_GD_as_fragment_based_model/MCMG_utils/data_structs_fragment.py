...
class MolData(Dataset):
    ...
    def __init__(self, fname, voc):
        ...
        with open(fname, 'r') as f:
            next(f)
            for line in f:
                self.smiles.append(line.split(',')[1])
...
class Experience(object):
    ...
    def add_experience(self, experience):
        ...
        """Raw Method: Experience should be a list of (smiles, score, prior likelihood) tuples"""
        ...
        #     best_score = float(self.memory[0][1])  # numpy float32数据在下行的格式输出容易出错，转换为python的浮点数
        #     print("\nBest score in memory: {:.2f}".format(best_score))
        
        
        
        
        """Modified Method: Experience should be a list of (smiles, score, prior likelihood) tuples"""
        self.memory.extend(experience)
        if len(self.memory)>self.max_size:
            # Remove duplicates
            idxs, smiles = [], []
            ...
            # Remove similarities
            smiles = [x[0] for x in self.memory]
            idxs_2 = []
            valid_mols = []
            for i, exp in enumerate(self.memory):
                try:
            ...
            self.memory = [self.memory[idx] for idx in idxs_2]

            # mols = [Chem.MolFromSmiles(exp[0]) for i in self.memory]

            if len(valid_mols)>1:
                #fps = [AllChem.GetMorganFingerprintAsBitVect(x, 3, 2048) for x in valid_mols]
                mfpgen = rdFingerprintGenerator.GetMorganGenerator(
                    radius=3,
                    fpSize=2048,
                    countSimulation=False,
                )
                fps = [mfpgen.GetFingerprint(x) for x in valid_mols]
                ...
                idxs_4 = list(set(idxs)-set(idxs_3))
                self.memory = [self.memory[idx] for idx in idxs_4]
            # Retain highest scores
            self.memory.sort(key = lambda x: x[1], reverse=True)
            self.memory = self.memory[:self.max_size]
            if self.memory:
    ...
    def sample(self, n):
        ...
        encoded = []
        for tokenized_i in tokenized:
            if tokenized_i != 'skip':
                try:
                    m = Variable(self.voc.encode(tokenized_i))
                except:
                    m = Variable(np.zeros(len(tokenized_i), dtype=np.float32))
            else:
                m = Variable(np.zeros(2, dtype=np.float32))
            encoded.append(m)
...
# def decode_molecule(enc):
#     fs = join_fragments(Chem.MolFromSmiles(enc))
#     return fs
